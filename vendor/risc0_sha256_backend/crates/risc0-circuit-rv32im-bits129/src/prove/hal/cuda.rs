...
use risc0_circuit_rv32im_bits129_sys::{
    risc0_circuit_rv32im_bits129_cuda_accum, risc0_circuit_rv32im_bits129_cuda_eval_check,
    risc0_circuit_rv32im_bits129_cuda_witgen, RawAccumBuffers, RawBuffer, RawExecBuffers,
    RawPreflightTrace,
};
use risc0_core::{
...
use risc0_zkp::{
    ...
    hal::{
        cuda::{
            ...
            CudaHashPoseidon2, CudaHashSha256,
        },
...
impl<CH: CudaHash> CircuitWitnessGenerator<CudaHal<CH>> for CudaCircuitHal<CH> {
    fn generate_witness(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_rv32im_bits129_cuda_witgen(
                mode as u32,
                &buffers,
                &preflight,
                cycles as u32,
...
impl<CH: CudaHash> CircuitAccumulator<CudaHal<CH>> for CudaCircuitHal<CH> {
    fn step_accum(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_rv32im_bits129_cuda_accum(&buffers, &preflight, cycles as u32)
        })
    }
}
...
impl<CH: CudaHash> CircuitHal<CudaHal<CH>> for CudaCircuitHal<CH> {
    ...
    fn eval_check(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_rv32im_bits129_cuda_eval_check(
                check.as_device_ptr(),
                ctrl.as_device_ptr(),
                data.as_device_ptr(),
                accum.as_device_ptr(),
...
pub fn segment_prover() -> Result<Box<dyn SegmentProver>> {
    let hash_suite = risc0_zkp::core::hash::poseidon2::Poseidon2HashSuite::new_suite();
    segment_prover_with_hash_suite(hash_suite)
}

pub fn segment_prover_with_hash_suite(
    hash_suite: HashSuite<BabyBear>,
) -> Result<Box<dyn SegmentProver>> {
    match hash_suite.name.as_str() {
        "poseidon2" => segment_prover_poseidon2(),
        "sha-256" => segment_prover_sha256(),
        other => bail!("unsupported rv32im CUDA hash suite: {other}"),
    }
}

fn segment_prover_poseidon2() -> Result<Box<dyn SegmentProver>> {
    let hal_factory = || {
...
fn segment_prover_sha256() -> Result<Box<dyn SegmentProver>> {
    let hal_factory = || {
