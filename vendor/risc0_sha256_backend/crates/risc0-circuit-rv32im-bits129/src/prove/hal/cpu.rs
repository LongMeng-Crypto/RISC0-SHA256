...
use risc0_circuit_rv32im_bits129_sys::{
    risc0_circuit_rv32im_bits129_cpu_accum, risc0_circuit_rv32im_bits129_cpu_poly_fp,
    risc0_circuit_rv32im_bits129_cpu_witgen, RawAccumBuffers, RawBabyBearExt6, RawBuffer,
    RawExecBuffers, RawPreflightTrace,
};
use risc0_core::scope;
use risc0_sys::ffi_wrap;
use risc0_zkp::{
    baby_bear_ext6::BabyBear6,
...
impl CircuitWitnessGenerator<CpuHal> for CpuCircuitHal {
    fn generate_witness(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_rv32im_bits129_cpu_witgen(
                mode as u32,
                &buffers,
                &preflight,
                cycles as u32,
...
impl CircuitAccumulator<CpuHal> for CpuCircuitHal {
    fn step_accum(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_rv32im_bits129_cpu_accum(&buffers, &preflight, cycles as u32)
        })
    }
}
...
impl CircuitHal<CpuHal> for CpuCircuitHal {
    fn eval_check(
        ...
        (0..domain).into_par_iter().for_each(|cycle| {
            ...
            unsafe {
                risc0_circuit_rv32im_bits129_cpu_poly_fp(
                    cycle,
                    domain,
                    poly_mix_pows.as_ptr().cast::<RawBabyBearExt6>(),
                    args.as_ptr(),
                    (&mut tot as *mut ExtVal).cast::<RawBabyBearExt6>(),
                )
            };
...
pub fn segment_prover() -> Result<Box<dyn SegmentProver>> {
    let hash_suite = risc0_zkp::core::hash::sha::Sha256HashSuite::<BabyBear6>::new_suite();
    segment_prover_with_hash_suite(hash_suite)
}

pub fn segment_prover_with_hash_suite(
    hash_suite: HashSuite<BabyBear6>,
) -> Result<Box<dyn SegmentProver>> {
    let hal_factory = move || {
        (
            Rc::new(CpuHal::new(hash_suite.clone())),
            Rc::new(CpuCircuitHal),
        )
    };
