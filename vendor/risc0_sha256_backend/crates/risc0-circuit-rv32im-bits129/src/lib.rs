...
pub const RV32IM_SEAL_VERSION: u32 = 0x8129_0001;
...
pub fn verify(seal: &[u32]) -> Result<(), VerificationError> {
    verify_with_hash_suite(
        seal,
        &risc0_zkp::core::hash::sha::Sha256HashSuite::<BabyBear6>::new_suite(),
    )
}

pub fn verify_with_hash_suite(
    seal: &[u32],
    ...
    risc0_zkp::verify::verify_with_queries(&CircuitImpl, hash_suite, seal, 65, check_code_fn)
}
