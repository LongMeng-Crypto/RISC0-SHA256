...
pub mod testutil {
    ...
    impl EvalCheckParams {
        pub fn new(po2: usize) -> Self {
            ...
            let poly_mix = BabyBearExt6::random(&mut rng);
    ...
    #[test]
    fn cpu_poly_fp_matches_poly_ext6() {
        use risc0_zkp::hal::cpu::CpuHal;
        use risc0_zkp::{
            adapter::PolyExt, baby_bear_ext6::BabyBear6, core::hash::sha::Sha256HashSuite,
            field::RootsOfUnity,
        };

        let params = EvalCheckParams::new(8);
        let hal = CpuHal::<BabyBear6>::new(Sha256HashSuite::<BabyBear6>::new_suite());
        let check = eval_check_impl(&params, &hal, &crate::prove::hal::cpu::CpuCircuitHal);
        let circuit = CircuitImpl::new();
        let taps = circuit.get_taps();
        let groups = [&params.accum[..], &params.code[..], &params.data[..]];

        for cycle in [0usize, 1, 17, params.domain - 1] {
            let eval_u = taps
                .taps()
                .map(|tap| {
                    let row = (cycle + params.domain - tap.back() * INV_RATE) % params.domain;
                    BabyBearExt6::from_subfield(
                        &groups[tap.group()][tap.offset() * params.domain + row],
                    )
                })
                .collect::<Vec<_>>();
            let result = circuit
                .poly_ext(&params.poly_mix, &eval_u, &[&params.out, &params.mix])
                .tot;
            let quotient = BabyBearExt6::from_subelems(
                (0..BabyBearExt6::EXT_SIZE)
                    .map(|component| check[component * params.domain + cycle]),
            );
            let x = BabyBearElem::ROU_FWD[params.po2 + 2].pow(cycle);
            let point = BabyBearElem::from_u64(3) * x;
            assert_eq!(
                result,
                quotient
                    * (BabyBearExt6::from_subfield(&point).pow(params.steps) - BabyBearExt6::ONE),
                "poly_fp/poly_ext mismatch at expanded-domain cycle {cycle}"
            );
        }
    }
    ...
    where
        H1: Hal<Elem = BabyBearElem, ExtElem = BabyBearExt6>,
        H2: Hal<Elem = BabyBearElem, ExtElem = BabyBearExt6>,
        C1: CircuitHal<H1>,
        C2: CircuitHal<H2>,
    {
    ...
    where
        H: Hal<Elem = BabyBearElem, ExtElem = BabyBearExt6>,
        C: CircuitHal<H>,
    {
        let check = hal.alloc_elem("check", BabyBearExt6::EXT_SIZE * params.domain);
