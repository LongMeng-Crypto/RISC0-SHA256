...
use risc0_circuit_recursion_bits129_sys::RawPreflightCycle;
use risc0_core::field::baby_bear::BabyBearElem as Fp;
use risc0_zkp::baby_bear_ext6::BabyBearExt6 as FpExt;
use risc0_zkp::{
    core::{
...
mod micro_op {
    ...
    pub const EXTRACT_LOW4: u32 = 10;
    pub const EXTRACT_HIGH2: u32 = 11;
}
...
impl Preflight {
    ...
    fn micro_op(
        ...
        let is_par_safe = match opcode {
            micro_op::CONST => {
                let result = FpExt::new([args[0], args[1], Fp::ZERO, Fp::ZERO, Fp::ZERO, Fp::ZERO]);
            ...
            micro_op::EQ => {
                ...
                if a != b {
                    bail!(
                        "Equality check failed at recursion cycle {cycle}, WOM {:?} vs {:?}: Expecting {a:?} == {b:?}",
                        args[0], args[1]
                    );
                }
                true
            ...
            micro_op::EXTRACT_LOW4 => {
                let a = self.wom_read(args[0]);
                let index = (args[1].as_u32() + 2 * args[2].as_u32()) as usize;
                ensure!(
                    index < 4,
                    "invalid low Fp6 coefficient index {index} at cycle {cycle}, args={args_u32:?}"
                );
                let val = a.elems()[index];
                tracing::trace!("extract_low4 {a:?}[{index}] -> {val:?}");
                self.wom_write(write_addr, val.into())?;
                true
            }
            micro_op::EXTRACT_HIGH2 => {
                let a = self.wom_read(args[0]);
                let index = 4 + args[1].as_u32() as usize;
                ensure!(
                    index < FpExt::EXT_SIZE,
                    "invalid high Fp6 coefficient index {index}"
                );
                let val = a.elems()[index];
                tracing::trace!("extract_high2 {a:?}[{index}] -> {val:?}");
                self.wom_write(write_addr, val.into())?;
                true
            }
