...
use risc0_circuit_recursion_bits129_sys::{
    risc0_circuit_recursion_bits129_cpu_accum, risc0_circuit_recursion_bits129_cpu_eval_check,
    risc0_circuit_recursion_bits129_cpu_witgen, RawAccumBuffers, RawBabyBearExt6, RawExecBuffers,
    RawPreflightTrace, StepMode,
};
use risc0_sys::ffi_wrap;
use risc0_zkp::{
    baby_bear_ext6::{BabyBear6, BabyBearExt6},
    core::{hash::sha::Sha256HashSuite, log2_ceil},
    field::{baby_bear::BabyBearElem, map_pow, RootsOfUnity as _},
...
type CpuHal = risc0_zkp::hal::cpu::CpuHal<BabyBear6>;
...
impl CircuitWitnessGenerator<CpuHal> for CpuCircuitHal {
    fn generate_witness(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_recursion_bits129_cpu_witgen(mode, &buffers, preflight, total_cycles)
        })
    }
}
...
impl CircuitAccumulator<CpuHal> for CpuCircuitHal {
    fn accumulate(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_recursion_bits129_cpu_accum(&buffers, work_cycles, total_cycles)
        })
    }
}
...
impl CircuitHal<CpuHal> for CpuCircuitHal {
    fn eval_check(
        ...
        ffi_wrap(|| unsafe {
            risc0_circuit_recursion_bits129_cpu_eval_check(
                &buffers,
...
pub(crate) fn recursion_prover(hashfn: &str) -> Result<Rc<dyn RecursionProver>> {
    let suite = match hashfn {
        "sha-256" => Sha256HashSuite::<BabyBear6>::new_suite(),
        _ => bail!("Bits129 recursion supports only the sha-256 backend, got {hashfn}"),
    };
