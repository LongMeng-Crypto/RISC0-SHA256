...
use risc0_circuit_recursion_bits129_sys::{RawBabyBearExt6, RawPreflightTrace, StepMode};
...
where
    H: Hal<Field = BabyBear6, Elem = BabyBearElem, ExtElem = BabyBearExt6>,
{
    pub fn new<C: CircuitWitnessGenerator<H>>(
        ...
        let raw_trace = RawPreflightTrace {
            wom: preflight.trace.wom.as_ptr().cast::<RawBabyBearExt6>(),
            cycles: preflight.trace.cycles.as_ptr(),
            iops: preflight.trace.iops.as_ptr().cast::<RawBabyBearExt6>(),
