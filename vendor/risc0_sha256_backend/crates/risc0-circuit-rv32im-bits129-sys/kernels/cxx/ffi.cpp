...
namespace risc0::circuit::rv32im_bits129::cpu {

constexpr size_t kUserAccumSplit = kLayout_TopAccum.columns[0].col;
constexpr size_t kExtSize = 6;
...
using namespace risc0::circuit::rv32im_bits129::cpu;

const char* risc0_circuit_rv32im_bits129_cpu_witgen(uint32_t mode,
                                            ExecBuffers* buffers,
                                            PreflightTrace* preflight,
                                            uint32_t lastCycle) {
  LookupTables tables;
...
const char* risc0_circuit_rv32im_bits129_cpu_accum(AccumBuffers* buffers,
                                           PreflightTrace* preflight,
                                           uint32_t lastCycle) {
  try {
    LookupTables tables;
...

    {
      ...
      for (size_t j = 0; j < kExtSize; j++) {
        size_t col = buffers->accum.cols - kExtSize + j;
...

    {
      ...
      size_t machineColumns = (buffers->accum.cols - kUserAccumSplit) / kExtSize;
      auto begin = poolstl::iota_iter<uint32_t>(0);
      auto end = poolstl::iota_iter<uint32_t>(lastCycle);
      std::for_each(poolstl::par, begin, end, [&](uint32_t row) {
        size_t back1 = (row + lastCycle - 1) % lastCycle;
        std::array<Fp, kExtSize> prev;
        for (size_t k = 0; k < kExtSize; k++) {
          prev[k] = buffers->accum.get(back1, buffers->accum.cols - kExtSize + k);
        }
        for (size_t j = 0; j < machineColumns - 1; j++) {
          for (size_t k = 0; k < kExtSize; k++) {
            size_t col = kUserAccumSplit + j * kExtSize + k;
