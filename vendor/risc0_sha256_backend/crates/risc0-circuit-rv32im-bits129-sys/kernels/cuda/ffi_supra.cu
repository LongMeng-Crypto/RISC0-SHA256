...
__constant__ FpExt6 poly_mix[kNumPolyMixPows];
...
__global__ void eval_check(Fp* check,
  ...
  if (cycle < domain) {
    FpExt6 tot = poly_fp(cycle, domain, ctrl, out, data, mix, accum);
    Fp x = pow(rou, cycle);
    Fp y = pow(Fp(3) * x, 1 << po2);
    FpExt6 ret = tot * inv(y - Fp(1));
...
using namespace risc0::circuit::rv32im_bits129::cuda;

extern "C" {

const char* risc0_circuit_rv32im_bits129_cuda_eval_check(Fp* check,
                                                 const Fp* ctrl,
                                                 const Fp* data,
                                                 const Fp* accum,
                                                 const Fp* mix,
