...
struct ContextAdapter<'a, 'b, C: CircuitSyscallContext> {
    ctx: &'b mut C,
    syscall_table: SyscallTable<'a>,
}

impl<'a, C: CircuitSyscallContext> SyscallContext<'a> for ContextAdapter<'a, '_, C> {
    fn get_pc(&self) -> u32 {
        self.ctx.get_pc()
    }
...
impl<C: CircuitSyscallContext> ContextAdapter<'_, '_, C> {
