...
const SHA256_RECURSION_PO2: usize = 21;
...
fn get_zkr(name: &str, hashfn: &str) -> Result<(Program, Digest)> {
    ...
    Ok((
        risc0_circuit_recursion::prove::zkr::get_zkr(&program_name, recursion_po2)?,
        control_ids
            .iter()
            .copied()
...
fn hash_specific_name(name: String, hashfn: &str) -> String {
    if hashfn != "sha-256" {
        return name;
    }

    if let Some(po2) = name
        .strip_prefix("lift_rv32im_v2_povw_")
        .and_then(|value| value.strip_suffix(".zkr"))
    {
        return format!("lift_rv32im_v2_povw_sha256_{po2}.zkr");
    }
    if let Some(po2) = name
        .strip_prefix("lift_rv32im_v2_")
        .and_then(|value| value.strip_suffix(".zkr"))
    {
        return format!("lift_rv32im_v2_sha256_{po2}.zkr");
    }
    match name.as_str() {
