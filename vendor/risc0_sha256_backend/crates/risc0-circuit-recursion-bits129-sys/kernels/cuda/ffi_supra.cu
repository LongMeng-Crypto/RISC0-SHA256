...
__global__ void eval_check(Fp* check,
  ...
  if (cycle < domain) {
    ...
    check[domain * 4 + cycle] = ret[4];
    check[domain * 5 + cycle] = ret[5];
  }
}

} // namespace risc0::circuit::recursion_bits129::cuda

extern "C" const char* risc0_circuit_recursion_bits129_cuda_eval_check(Fp* check,
                                                               const Fp* ctrl,
                                                               const Fp* data,
                                                               const Fp* accum,
                                                               const Fp* mix,
  ...
  try {
    ...
    risc0::circuit::recursion_bits129::cuda::eval_check<<<cfg.grid, cfg.block, 0, stream>>>(
