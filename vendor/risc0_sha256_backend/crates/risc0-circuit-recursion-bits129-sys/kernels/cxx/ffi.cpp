...
using namespace risc0::circuit::recursion_bits129;
  ...
  case kStepModeSeqForward: {
    for (size_t i = 0; i < trace->numCycles; i++) {
      try {
        ...
        throw std::runtime_error("step_verify_mem cycle " + std::to_string(i) + ": " + err.what());
      }
    }
  } break;
  case kStepModeSeqReverse: {
...
void MachineContext::injectWomBacks() {
  ...
  std::for_each(poolstl::par, begin, end, [&](uint32_t cycle) {
    ...
    if (idx) {
      ...
      data[5 * steps + cycle - 1] = prev.value.elems[4];
      data[6 * steps + cycle - 1] = prev.value.elems[5];
    } else {
      ...
      data[5 * steps + cycle - 1] = 0;
      data[6 * steps + cycle - 1] = 0;
    }
  });
}
...
const char* risc0_circuit_recursion_bits129_cpu_witgen(uint32_t mode,
                                               ExecBuffers* buffers,
...
const char*
risc0_circuit_recursion_bits129_cpu_accum(AccumBuffers* buffers, uint32_t steps, uint32_t cycles) {
  try {
...
const char* risc0_circuit_recursion_bits129_cpu_eval_check(
  ...
  try {
    ...
    std::for_each(poolstl::par, begin, end, [&](uint32_t cycle) {
      ...
      check[steps * 4 + cycle] = ret.elems[4];
      check[steps * 5 + cycle] = ret.elems[5];
    });
  } catch (const std::exception& err) {
    return strdup(err.what());
  }
