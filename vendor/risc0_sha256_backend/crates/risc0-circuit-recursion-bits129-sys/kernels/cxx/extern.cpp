...
void extern_womWrite(void* ctx, size_t cycle, const char* extra, std::array<Fp, 7> args) {
  // uint32_t addr = args[0].asUInt32();
...
void extern_plonkWrite_wom(void* ctx, size_t cycle, const char* extra, std::array<Fp, 7> args) {
  MachineContext* mctx = static_cast<MachineContext*>(ctx);
  uint32_t addr = args[0].asUInt32();
  FpExt value(args[1], args[2], args[3], args[4], args[5], args[6]);
...
void extern_plonkWriteAccum_wom(void* ctx,
  ...
  actx->accum[cycle] = FpExt(args[0], args[1], args[2], args[3], args[4], args[5]);
}
