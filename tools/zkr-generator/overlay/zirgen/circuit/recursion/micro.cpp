...
void MicroOpImpl::set(MicroInst inst, Val writeAddr, Reg extraPrev, size_t extraBack) {
  ...
  IF(decode->at(size_t(MicroOpcode::INV)) * (1 - operands[1])) {
    ...
    NONDET {
      extra->set(inv(input));
      out->doWrite(writeAddr, FpExt(1 - extra * input).getElems());
    }
    Val maybeInv = extra;
  ...
  IF(decode->at(size_t(MicroOpcode::READ_IOP_HEADER))) {
    ...
    in0->doNOP();
    in1->doNOP();
    out->doWrite(writeAddr, FpExt(Val(0)).getElems());
  ...
  IF(decode->at(size_t(MicroOpcode::MIX_RNG))) {
    ...
    XLOG("  val = %u", val);
    extra->set(val);
    out->doWrite(writeAddr, FpExt(val).getElems());
  }
  ...
  IF(decode->at(size_t(MicroOpcode::EXTRACT_LOW4))) {
    XLOG("%u> EXTRACT_LOW4: %e", writeAddr, operands);
    in0->doRead(operands[0]);
    in1->doNOP();
    Val lowBit = operands[1];
    Val highBit = operands[2];
    extractMux[0]->set((1 - lowBit) * in0->data()[0] + lowBit * in0->data()[1]);
    extractMux[1]->set((1 - lowBit) * in0->data()[2] + lowBit * in0->data()[3]);
    extractMux[2]->set((1 - highBit) * extractMux[0] + highBit * extractMux[1]);
    out->doWrite(writeAddr, FpExt(extractMux[2]->get()).getElems());
  }
  IF(decode->at(size_t(MicroOpcode::EXTRACT_HIGH2))) {
    XLOG("%u> EXTRACT_HIGH2: %e", writeAddr, operands);
    in0->doRead(operands[0]);
    in1->doNOP();
    Val lowBit = operands[1];
    extractMux[0]->set((1 - lowBit) * in0->data()[4] + lowBit * in0->data()[5]);
    extractMux[1]->set(0);
    extractMux[2]->set(0);
    out->doWrite(writeAddr, FpExt(extractMux[0]->get()).getElems());
  }
}
