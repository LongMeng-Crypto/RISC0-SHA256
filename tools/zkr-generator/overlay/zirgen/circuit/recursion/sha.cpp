...
void ShaCycleImpl::setFini(MacroInst inst, Val writeAddr) {
  ...
  io0->doWrite(inst->operands[0],
               FpExt::fromVals({outA[3] + 256 * outA[2], outA[1] + 256 * outA[0], 0, 0, 0, 0})
                   .getElems());
  io1->doWrite(inst->operands[1],
               FpExt::fromVals({outE[3] + 256 * outE[2], outE[1] + 256 * outE[0], 0, 0, 0, 0})
                   .getElems());
}
