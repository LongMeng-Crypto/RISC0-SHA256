...
static std::array<Val, kBabyBearExtSize> readExtVal(llvm::ArrayRef<Val>& stream) {
  assert(stream.size() >= kBabyBearExtSize);
  std::array<Val, kBabyBearExtSize> out;
  for (size_t i = 0; i < kBabyBearExtSize; i++) {
    out[i] = stream[i];
  }
  stream = stream.drop_front(kBabyBearExtSize);
  return out;
}
