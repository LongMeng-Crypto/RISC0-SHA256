...
VerifyInfo verify(ReadIopVal& iop, size_t po2, const CircuitInterface& circuit) {
  ...
  Val check = 0;
  size_t remap[kInvRate] = {0, 2, 1, 3};
  for (size_t i = 0; i < kInvRate; i++) {
    Val zi = raisepow(Z, i);
    size_t rmi = remap[i];
    for (size_t component = 0; component < kBabyBearExtSize; component++) {
      std::vector<uint64_t> basis(kBabyBearExtSize, 0);
      basis[component] = 1;
      check = check + coeffU[tapCount + component * kInvRate + rmi] * zi * Val(basis);
    }
  }
