...
void emitRecursion(const std::string& path, func::FuncOp func, recursion::EncodeStats* stats) {
  ...
  const auto hashType = name.find("sha256") == std::string::npos
                            ? recursion::HashType::POSEIDON2
                            : recursion::HashType::SHA256;
  auto encoded = recursion::encode(hashType, &func.front(), &toId, stats);
