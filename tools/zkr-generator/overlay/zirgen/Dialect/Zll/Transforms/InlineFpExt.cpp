...
struct Rewriter {
  ...
  void doMul(MulOp op) {
    ...
    SmallVector<Value, 6> product(2 * kExt - 1, zero);
    for (size_t i = 0; i < kLhs; i++) {
      for (size_t j = 0; j < kRhs; j++) {
        size_t pos = i + j;
        Value mul = builder.create<MulOp>(op.getLoc(), lhsExp[i], rhsExp[j]);
        product[pos] = builder.create<AddOp>(op.getLoc(), product[pos], mul);
      }
    }
    SmallVector<Value, 6> out(product.begin(), product.begin() + kExt);
    if (kExt == 6) {
      // BabyBearExt6 is Fp[X] / (X^6 + X + 6), so X^6 = -X - 6.
      for (size_t degree = product.size(); degree-- > kExt;) {
        Value coeff = product[degree];
        size_t base = degree - kExt;
        Value six = builder.create<ConstOp>(op.getLoc(), zero.getType(), 6);
        Value sixCoeff = builder.create<MulOp>(op.getLoc(), six, coeff);
        out[base] = builder.create<SubOp>(op.getLoc(), out[base], sixCoeff);
        if (base + 1 < kExt) {
          out[base + 1] = builder.create<SubOp>(op.getLoc(), out[base + 1], coeff);
        } else {
          product[base + 1] =
              builder.create<SubOp>(op.getLoc(), product[base + 1], coeff);
        }
      }
    } else {
      // Legacy degree-four extension: X^4 = -11.
      for (size_t degree = kExt; degree < product.size(); degree++) {
        Value reduced = builder.create<MulOp>(op.getLoc(), nbeta, product[degree]);
        out[degree - kExt] =
            builder.create<AddOp>(op.getLoc(), out[degree - kExt], reduced);
      }
    }
    expansions[op.getOut()].assign(out.begin(), out.end());
  }
